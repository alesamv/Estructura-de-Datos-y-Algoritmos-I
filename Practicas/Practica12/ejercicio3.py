#Sudoku
"""Programa que resuelve el juego de Sudoku"""

from random import choice
def number_in_row(grid, row, number):
    """
    Revisa si un número se encuentra en la fila especificada.
    """
    return number in grid[row]
    
def number_in_col(grid, col, number):
    """
    Revisa si un número se encuentra en la columna especificada.
    """
    return number in (row[col] for row in grid)

def number_in_box(grid, row, col, number):
    """
    Revisa si un número se encuentra en la caja a la que
